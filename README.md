...
Each private iMessage conversation starts with a durable `Main` session. New sessions
have separate Eve histories and pending requests, so their context does not mix.

Asking to create, show, switch, rename, archive, restore, reset, or otherwise manage a
session opens the owner-bound Spectrum session manager without a model-written reply.
The older word `workspace` is accepted as an alias, but the UI and Eve's replies use
`session`. All changes happen in the manager, including **Start fresh**, which retires
only the selected session's model history.

The manager link is a short-lived capability carried in the URL fragment, and every
