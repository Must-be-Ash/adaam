...
- [ ] Pause/archive `Inverse Cramer U1 Final Green 0821` with a refreshed owner token, deploy the compact-actionability repair, then perform one fresh zero-usage `inverse-cramer@1.4.5` Production acceptance. Do not begin U2 until that one occurrence terminalizes, actual cost is separated from reservations, and cleanup proves every disposable monitor non-dispatchable.
