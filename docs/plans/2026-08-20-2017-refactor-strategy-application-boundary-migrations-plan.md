...
- [ ] Root-cause and repair `source_outside_fence` on a cadence window whose shared canonical facts arrive as corrections, then deploy and perform one new zero-usage acceptance. The Production trace shows acquisition completing and recording two corrections before the fence rejected a same-run source fetch, and `authorizeWorkspaceSourceFetch` rejects once the coverage record leaves `evaluating`; the local two-statement fixture has no prior revisions and therefore does not reproduce it. Do not reuse the failed occurrence and do not begin U2 until U1 is green.
