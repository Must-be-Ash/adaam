...
- [ ] Decide and implement the nested fan-out sizing repair, then deploy and perform one new zero-usage acceptance. The occurrence envelope funds exactly one compact semantic child while the strategy fans out over every eligible post in the cadence window, so either the parent envelope must cover the declared concurrency through a new immutable pack version or an unfundable statement must be deferred to the next occurrence instead of failing the run. Do not reuse the failed occurrence and do not begin U2 until U1 is green.
- [ ] Repair deployed, Production health verified, and one new zero-usage acceptance terminal and reported.
