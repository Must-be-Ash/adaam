...
import { createPublicCommentaryResearchDefinition } from "../agent/lib/public-commentary-research";
...
assert.equal(
  targetPack.evidenceContracts?.find(({ id }) => id === "public-commentary-frontier-research")?.digest,
  createPublicCommentaryResearchDefinition(["openai/gpt-5.4-mini"], "1.0.1").definitionDigest,
);
const withoutResearch = (pack: typeof sourcePack) => pack.evidenceContracts
  ?.filter(({ id }) => id !== "public-commentary-frontier-research");
assert.deepEqual(withoutResearch(targetPack), withoutResearch(sourcePack));
...
assert.ok(monitor.lifecycleState === "enabled" || monitor.lifecycleState === "paused");
...
const nextStrategy = {
  ...
  managedResources: strategy.value.managedResources,
...
if (apply) {
  ...
  const currentRaws = await Promise.all(keys.slice(0, 3).map((key) => redis.get<string>(key)));
  assert.ok(currentRaws.every((raw): raw is string => typeof raw === "string"));
  assert.deepEqual(currentRaws, [
    JSON.stringify(capabilities),
    JSON.stringify(strategy),
    JSON.stringify(monitor),
  ], "workspace changed after migration planning");
...
local replayed = true
for index = 1, 3 do
  if redis.call("GET", KEYS[index]) ~= ARGV[index + 3] then replayed = false end
end
if replayed then return "replayed" end
for index = 1, 3 do
  if redis.call("GET", KEYS[index]) ~= ARGV[index] then return "conflict" end
end
...
`, keys, [
  ...
  console.info("public commentary research-model migration committed");
}
