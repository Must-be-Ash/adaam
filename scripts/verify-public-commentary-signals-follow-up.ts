...
const versions = strategyPackCatalog.entries
  ...
  .map(({ version }) => version);
assert.deepEqual(versions, ["1.0.0", "1.1.0", "1.2.0", "1.3.0", "1.4.0", "1.4.1", "1.4.2", "1.4.3", "1.4.4", "1.4.5", "1.4.6", "1.4.7", "1.4.8"]);
assert.equal(
...
// The occurrence envelope must fund every nested semantic child projected from
// one cadence window, not a single model call. Historical packs keep theirs.
assert.equal(latestPack.monitors[0]?.suggestedBudget.maximumInputTokensPerRun, 280_000);
assert.equal(latestPack.monitors[0]?.suggestedBudget.maximumOutputTokensPerRun, 56_000);
assert.equal(historicalPack.monitors[0]?.suggestedBudget.maximumInputTokensPerRun, 40_000);
assert.equal(historicalPack.monitors[0]?.suggestedBudget.maximumOutputTokensPerRun, 12_000);
assert.equal(
  strategyPackCatalog.resolve({ id: "inverse-cramer", version: "1.4.5" })
    ?.monitors[0]?.suggestedBudget.maximumInputTokensPerRun,
  40_000,
);
